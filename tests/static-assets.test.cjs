...
assert.ok(!accountScript.includes("auth.mfa"));
...
assert.ok(!adminScript.includes("auth.mfa"));
assert.ok(!adminScript.includes("getAuthenticatorAssuranceLevel"));
...
const activitiesScript = read("assets/js/activities.js");
